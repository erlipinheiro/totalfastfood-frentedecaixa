object frmSelecionaPromocoes: TfrmSelecionaPromocoes
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'Sele'#231#227'o de promo'#231#245'es'
  ClientHeight = 768
  ClientWidth = 1024
  Color = 4259584
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlBotoes: TPanel
    Left = 0
    Top = 662
    Width = 1024
    Height = 106
    Align = alBottom
    BevelOuter = bvNone
    ParentBackground = False
    TabOrder = 0
    object btnVoltar: TImage
      AlignWithMargins = True
      Left = 28
      Top = 20
      Width = 73
      Height = 61
      Center = True
      Picture.Data = {
        
        A11D90A8D07193A92DD9FFD9}
      OnClick = btnVoltarClick
    end
    object btnOK: TPanel
      Left = 802
      Top = 8
      Width = 185
      Height = 90
      BevelOuter = bvNone
      Caption = 'OK'
      Color = 16744448
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -64
      Font.Name = 'Roboto'
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      OnClick = btnOKClick
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 41
    Width = 1024
    Height = 621
    Align = alClient
    DataSource = dsPromocoes
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -43
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDblClick = DBGrid1DblClick
    Columns = <
      item
        Expanded = False
        FieldName = 'Descricao'
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1024
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Selecione abaixo a promo'#231#227'o'
    Color = 16744448
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -24
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 2
  end
  object qryPromocoes: TFDQuery
    Connection = dmCaixa.dbTotalFastFood
    SQL.Strings = (
      'select p.* from Vendas.EmpresaPromocoes ep '
      'inner join Produtos.Promocoes p on ep.idPromocao=p.idPromocao '
      
        'where idEmpresa = :idEmpresa and ep.Ativa = 1 and p.ATIVA=1 and ' +
        ' p.AtivacaoAutomatica = 0 and p.ExigirCupomDesconto = 0'
      'order by Descricao'
      '')
    Left = 516
    Top = 217
    ParamData = <
      item
        Name = 'IDEMPRESA'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
  end
  object dsPromocoes: TDataSource
    DataSet = qryPromocoes
    Left = 518
    Top = 267
  end
end
