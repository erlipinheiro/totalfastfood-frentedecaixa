object fSelecionaConvenio: TfSelecionaConvenio
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'Seleciona conv'#234'nio'
  ClientHeight = 768
  ClientWidth = 1024
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 703
    Width = 1024
    Height = 65
    Align = alBottom
    BevelOuter = bvNone
    Color = 13690861
    ParentBackground = False
    TabOrder = 0
    object btnVoltar: TImage
      AlignWithMargins = True
      Left = 8
      Top = 2
      Width = 73
      Height = 61
      Center = True
      Picture.Data = {
        
        A11D90A8D07193A92DD9FFD9}
      OnClick = btnVoltarClick
    end
    object btnOK: TPanel
      Left = 875
      Top = 5
      Width = 121
      Height = 55
      Caption = 'OK'
      Color = 4210816
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 13690861
      Font.Height = -11
      Font.Name = 'Roboto'
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      OnClick = btnOKClick
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 1024
    Height = 54
    Align = alTop
    BevelOuter = bvNone
    Color = clMaroon
    ParentBackground = False
    TabOrder = 1
    object lblSelecione: TLabel
      Left = 7
      Top = 0
      Width = 476
      Height = 51
      Caption = 'Selecione um conv'#234'nio'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -43
      Font.Name = 'Roboto'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 54
    Width = 1024
    Height = 649
    Align = alClient
    DataSource = dsConvenios
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'Descricao'
        Title.Caption = 'Descri'#231#227'o'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Ativo'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RazaoSocial'
        Title.Caption = 'Raz'#227'o social'
        Width = 340
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NomeFantasia'
        Title.Caption = 'Nome de fantasia'
        Visible = True
      end>
  end
  object qryConvenios: TFDQuery
    Connection = dmCaixa.dbTotalFastFood
    SQL.Strings = (
      
        'select c.*, p.RazaoSocial, p.NomeFantasia from vendas.Convenios ' +
        'c '
      'inner join Contabil.Pessoas p on c.idPessoa = p.idPessoa'
      'where c.ativo = 1 and '
      
        'exists (select 1 from Vendas.EmpresaPessoaConvenios where idempr' +
        'esa = :idEmpresa and idConvenio = c.idConvenio)')
    Left = 430
    Top = 180
    ParamData = <
      item
        Name = 'idEmpresa'
        DataType = ftInteger
        ParamType = ptInput
        Value = 21
      end>
    object qryConveniosidConvenio: TIntegerField
      FieldName = 'idConvenio'
      Required = True
    end
    object qryConveniosDescricao: TStringField
      FieldName = 'Descricao'
      Size = 50
    end
    object qryConveniosDataInicio: TDateTimeField
      FieldName = 'DataInicio'
    end
    object qryConveniosDataFim: TDateTimeField
      FieldName = 'DataFim'
    end
    object qryConveniosAtivo: TBooleanField
      FieldName = 'Ativo'
    end
    object qryConveniosidPessoa: TIntegerField
      FieldName = 'idPessoa'
    end
    object qryConveniosRazaoSocial: TStringField
      FieldName = 'RazaoSocial'
      ReadOnly = True
      Required = True
      Size = 80
    end
    object qryConveniosNomeFantasia: TStringField
      FieldName = 'NomeFantasia'
      ReadOnly = True
      Size = 50
    end
  end
  object dsConvenios: TDataSource
    DataSet = qryConvenios
    Left = 430
    Top = 230
  end
end
