object frmAberturaSessaoCaixa: TfrmAberturaSessaoCaixa
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'Abertura de Sess'#227'o de Caixa'
  ClientHeight = 768
  ClientWidth = 1024
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlCaixa: TPanel
    Left = 0
    Top = 0
    Width = 1024
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Abertura do Turno'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -43
    Font.Name = 'Roboto'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
  end
  object pnlDataCaixa: TPanel
    Left = 0
    Top = 41
    Width = 1024
    Height = 604
    Align = alClient
    BevelOuter = bvNone
    Caption = '31/10/13'
    Color = 8421440
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -240
    Font.Name = 'Roboto'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    object lblConfiraData: TLabel
      Left = 0
      Top = 6
      Width = 1016
      Height = 175
      Alignment = taCenter
      AutoSize = False
      Caption = 'Confira a data'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -160
      Font.Name = 'Roboto'
      Font.Style = []
      ParentFont = False
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 703
    Width = 1024
    Height = 65
    Align = alBottom
    BevelOuter = bvNone
    Color = 13690861
    ParentBackground = False
    TabOrder = 2
    object btnVoltar: TImage
      AlignWithMargins = True
      Left = 8
      Top = 2
      Width = 73
      Height = 61
      Center = True
      Picture.Data = {
        
        A11D90A8D07193A92DD9FFD9}
      OnClick = btnVoltarClick
    end
    object btnSalvar: TPanel
      Left = 820
      Top = 5
      Width = 176
      Height = 55
      Caption = 'Abrir turno'
      Color = 4210816
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 13690861
      Font.Height = -19
      Font.Name = 'Roboto'
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      OnClick = btnSalvarClick
    end
  end
  object pnlFundoCaixa: TPanel
    Left = 0
    Top = 645
    Width = 1024
    Height = 58
    Align = alBottom
    BevelOuter = bvNone
    Color = 33023
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Roboto'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 3
    object Label1: TLabel
      Left = 674
      Top = 20
      Width = 88
      Height = 19
      Caption = 'Troco inicial'
    end
    object edtTrocoInicial: TEdit
      Left = 765
      Top = 6
      Width = 251
      Height = 43
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -29
      Font.Name = 'Roboto'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnChange = edtTrocoInicialChange
      OnKeyPress = edtTrocoInicialKeyPress
    end
  end
end
