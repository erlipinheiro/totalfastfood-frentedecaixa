object frmSessaoCaixaApuracao: TfrmSessaoCaixaApuracao
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'Aupra'#231#227'o de Sess'#227'o de Caixa'
  ClientHeight = 768
  ClientWidth = 1024
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 703
    Width = 1024
    Height = 65
    Align = alBottom
    BevelOuter = bvNone
    Color = 13690861
    ParentBackground = False
    TabOrder = 0
    object btnVoltar: TImage
      AlignWithMargins = True
      Left = 8
      Top = 2
      Width = 73
      Height = 61
      Center = True
      Picture.Data = {
        
        A11D90A8D07193A92DD9FFD9}
      OnClick = btnVoltarClick
    end
    object btnSalvar: TPanel
      Left = 875
      Top = 5
      Width = 121
      Height = 55
      Caption = 'SALVAR'
      Color = 4210816
      Font.Charset = DEFAULT_CHARSET
      Font.Color = 13690861
      Font.Height = -11
      Font.Name = 'Roboto'
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      OnClick = btnSalvarClick
    end
  end
  object pnlTitulo: TPanel
    Left = 0
    Top = 0
    Width = 1024
    Height = 51
    Align = alTop
    Caption = 'Apura'#231#227'o do turno'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -37
    Font.Name = 'Roboto'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 51
    Width = 1024
    Height = 652
    Align = alClient
    DataSource = dsApuracao
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -24
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Color = 15263976
        Expanded = False
        FieldName = 'Descricao'
        Title.Caption = 'Forma de pagamento'
        Width = 346
        Visible = True
      end
      item
        Color = 15263976
        Expanded = False
        FieldName = 'Imagem'
        Title.Caption = 'Logo'
        Visible = True
      end
      item
        Color = 15263976
        Expanded = False
        FieldName = 'DataHora'
        ReadOnly = True
        Title.Caption = 'Data e hora'
        Width = 283
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlApurado'
        Title.Caption = 'Valor apurado'
        Width = 207
        Visible = True
      end>
  end
  object LinkLabel1: TLinkLabel
    Left = 792
    Top = 176
    Width = 53
    Height = 17
    Caption = 'LinkLabel1'
    TabOrder = 3
  end
  object qryApuracao: TFDQuery
    Connection = dmCaixa.dbTotalFastFood
    UpdateOptions.AssignedValues = [uvUpdateMode, uvCountUpdatedRecords]
    UpdateOptions.UpdateMode = upWhereAll
    UpdateOptions.CountUpdatedRecords = False
    UpdateOptions.KeyFields = 'idEmpresa;idCaixa;idSessaoCaixa;idFormaPagamento'
    SQL.Strings = (
      
        'Select sca.*, fp.Descricao, fp.Imagem from Vendas.SessaoCaixaApu' +
        'racoes  sca'
      
        'inner join Vendas.FormasPagamento fp on sca.idFormaPagamento = f' +
        'p.idFormaPagamento'
      
        'where sca.idEmpresa = 21 and sca.idCaixa = 1 and sca.idSessaoCai' +
        'xa = 1')
    Left = 485
    Top = 315
    object qryApuracaoidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryApuracaoidCaixa: TIntegerField
      FieldName = 'idCaixa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryApuracaoidSessaoCaixa: TIntegerField
      FieldName = 'idSessaoCaixa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryApuracaoidFormaPagamento: TIntegerField
      FieldName = 'idFormaPagamento'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryApuracaovlApurado: TCurrencyField
      FieldName = 'vlApurado'
    end
    object qryApuracaovlRealizado: TCurrencyField
      FieldName = 'vlRealizado'
    end
    object qryApuracaoDataHora: TDateTimeField
      FieldName = 'DataHora'
    end
    object qryApuracaoDescricao: TStringField
      FieldName = 'Descricao'
      ProviderFlags = []
      ReadOnly = True
    end
    object qryApuracaoImagem: TBlobField
      FieldName = 'Imagem'
      ProviderFlags = []
      ReadOnly = True
    end
  end
  object dsApuracao: TDataSource
    DataSet = qryApuracao
    Left = 485
    Top = 360
  end
  object qryProdutosVendidos: TFDQuery
    Connection = dmCaixa.dbTotalFastFood
    Left = 485
    Top = 411
  end
end
