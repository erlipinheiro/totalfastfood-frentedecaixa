...
//  NFe.ide.tpNF    := StrToTpNF(ok, LerCampo(Grupo, 'Tipo da Operação',1));
  ...
  if RightStr(Trim(txt),1) = ',' then
    txt := Copy(txt, 1, Length(txt) -1);

  if txt[1] = ' ' then
    txt := copy(txt,2, length(txt) -1);

  NFe.Emit.EnderEmit.nro := copy(txt, 1, pos(' ', txt) -1);
  NFe.Emit.EnderEmit.xCpl := copy(txt, pos(' ', txt), length(txt) -1);
...
//                  Prod.genero := StrToIntDef(LerCampo(Grupo,'Gênero'),0);
  ...
  try
//     GeradorXML.Gerador.Opcoes. := TsPL005c;
     GeradorXML.GerarXml;
