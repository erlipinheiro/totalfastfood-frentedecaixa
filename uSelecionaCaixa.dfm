object frmSelecionaCaixa: TfrmSelecionaCaixa
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'Seleciona caixa'
  ClientHeight = 728
  ClientWidth = 1011
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object dbgridCaixas: TDBGrid
    Left = 0
    Top = 41
    Width = 1011
    Height = 225
    Align = alClient
    DataSource = dsCaixas
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDblClick = btnOKClick
    Columns = <
      item
        Expanded = False
        FieldName = 'idCaixa'
        Title.Caption = 'ID'
        Width = 82
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dhAbertura'
        Title.Caption = 'Abertura'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dhFechamento'
        Title.Caption = 'Fechamento'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlTotalVenda'
        Title.Caption = 'Valor total'
        Width = 146
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'UsuarioAbertura'
        Title.Caption = 'Usu'#225'rio de abertura'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'UsuarioFechamento'
        Title.Caption = 'Usu'#225'rio de fechamento'
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1011
    Height = 41
    Align = alTop
    BevelOuter = bvNone
    Caption = 'Selecione abaixo o caixa'
    Color = 16744448
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -24
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
    ExplicitWidth = 852
  end
  object pnlBotoes: TPanel
    Left = 0
    Top = 622
    Width = 1011
    Height = 106
    Align = alBottom
    BevelOuter = bvNone
    ParentBackground = False
    TabOrder = 2
    ExplicitTop = 317
    ExplicitWidth = 852
    object btnVoltar: TImage
      AlignWithMargins = True
      Left = 28
      Top = 20
      Width = 73
      Height = 61
      Center = True
      Picture.Data = {
        
        A11D90A8D07193A92DD9FFD9}
      OnClick = btnVoltarClick
    end
    object btnOK: TPanel
      Left = 802
      Top = 8
      Width = 185
      Height = 90
      BevelOuter = bvNone
      Caption = 'OK'
      Color = 16744448
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -64
      Font.Name = 'Roboto'
      Font.Style = []
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      OnClick = btnOKClick
    end
  end
  object dbgridTurnos: TDBGrid
    Left = 0
    Top = 307
    Width = 1011
    Height = 315
    Align = alBottom
    DataSource = dsTurnos
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Visible = False
    OnDblClick = btnOKClick
    Columns = <
      item
        Expanded = False
        FieldName = 'idSessaoCaixa'
        Title.Caption = 'ID'
        Width = 82
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dhAbertura'
        Title.Caption = 'Abertura'
        Width = 104
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'dhFechamento'
        Title.Caption = 'Fechamento'
        Width = 93
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlInicial'
        Title.Caption = 'Troco inicial'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'vlVenda'
        Title.Caption = 'Valor total'
        Width = 146
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'UsuarioAbertura'
        Title.Caption = 'Usu'#225'rio de abertura'
        Width = 64
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'UsuarioFechamento'
        Title.Caption = 'Usu'#225'rio de fechamento'
        Visible = True
      end>
  end
  object pnlSelecioneTurno: TPanel
    Left = 0
    Top = 266
    Width = 1011
    Height = 41
    Align = alBottom
    BevelOuter = bvNone
    Caption = 'Selecione abaixo o turno'
    Color = 4227327
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -24
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 4
    Visible = False
    ExplicitLeft = -8
  end
  object qryCaixas: TFDQuery
    Connection = dmCaixa.dbTotalFastFood
    SQL.Strings = (
      
        'select c.*, pa.RazaoSocial as UsuarioAbertura, pf.RazaoSocial as' +
        ' UsuarioFechamento from vendas.caixas c'
      
        'inner join Contabil.Pessoas pa on c.idUsuarioAbertura = pa.idPes' +
        'soa'
      
        'inner join Contabil.Pessoas pf on c.idUsuarioFechamento = pf.idP' +
        'essoa'
      'where idempresa = :idEmpresa')
    Left = 476
    Top = 129
    ParamData = <
      item
        Name = 'IDEMPRESA'
        DataType = ftInteger
        ParamType = ptInput
        Value = 75
      end>
    object qryCaixasidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
      Origin = 'idEmpresa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryCaixasidCaixa: TIntegerField
      FieldName = 'idCaixa'
      Origin = 'idCaixa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryCaixasidUsuarioAbertura: TIntegerField
      FieldName = 'idUsuarioAbertura'
      Origin = 'idUsuarioAbertura'
      Required = True
    end
    object qryCaixasidUsuarioFechamento: TIntegerField
      FieldName = 'idUsuarioFechamento'
      Origin = 'idUsuarioFechamento'
    end
    object qryCaixasidListaPreco: TIntegerField
      FieldName = 'idListaPreco'
      Origin = 'idListaPreco'
      Required = True
    end
    object qryCaixasdhAbertura: TDateTimeField
      FieldName = 'dhAbertura'
      Origin = 'dhAbertura'
      Required = True
    end
    object qryCaixasdhFechamento: TDateTimeField
      FieldName = 'dhFechamento'
      Origin = 'dhFechamento'
    end
    object qryCaixasvlInicio: TCurrencyField
      FieldName = 'vlInicio'
      Origin = 'vlInicio'
    end
    object qryCaixasvlTotalVenda: TCurrencyField
      FieldName = 'vlTotalVenda'
      Origin = 'vlTotalVenda'
    end
    object qryCaixasvlDeposito: TCurrencyField
      FieldName = 'vlDeposito'
      Origin = 'vlDeposito'
    end
    object qryCaixasvlGastos: TCurrencyField
      FieldName = 'vlGastos'
      Origin = 'vlGastos'
    end
    object qryCaixasvlTroco: TCurrencyField
      FieldName = 'vlTroco'
      Origin = 'vlTroco'
    end
    object qryCaixasDataEnvioCentral: TDateTimeField
      FieldName = 'DataEnvioCentral'
      Origin = 'DataEnvioCentral'
    end
    object qryCaixasflEnviadoCentral: TBooleanField
      FieldName = 'flEnviadoCentral'
      Origin = 'flEnviadoCentral'
    end
    object qryCaixasUsuarioAbertura: TStringField
      FieldName = 'UsuarioAbertura'
      Origin = 'UsuarioAbertura'
      Required = True
      Size = 80
    end
    object qryCaixasUsuarioFechamento: TStringField
      FieldName = 'UsuarioFechamento'
      Origin = 'UsuarioFechamento'
      Required = True
      Size = 80
    end
  end
  object dsCaixas: TDataSource
    DataSet = qryCaixas
    Left = 480
    Top = 176
  end
  object qryTurnos: TFDQuery
    MasterSource = dsCaixas
    MasterFields = 'idEmpresa;idCaixa'
    Connection = dmCaixa.dbTotalFastFood
    SQL.Strings = (
      
        'select sc.*, pa.RazaoSocial as UsuarioAbertura, pf.RazaoSocial a' +
        's UsuarioFechamento from vendas.SessoesCaixa sc'
      
        'inner join Contabil.Pessoas pa on sc.idUsuarioAbertura = pa.idPe' +
        'ssoa'
      
        'inner join Contabil.Pessoas pf on sc.idUsuarioFechamento = pf.id' +
        'Pessoa'
      'where idempresa = :idEmpresa and sc.idCaixa = :idCaixa')
    Left = 606
    Top = 129
    ParamData = <
      item
        Name = 'IDEMPRESA'
        DataType = ftInteger
        ParamType = ptInput
        Value = 75
      end
      item
        Name = 'IDCAIXA'
        DataType = ftInteger
        ParamType = ptInput
        Value = 1
      end>
    object qryTurnosidEmpresa: TIntegerField
      FieldName = 'idEmpresa'
      Origin = 'idEmpresa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryTurnosidCaixa: TIntegerField
      FieldName = 'idCaixa'
      Origin = 'idCaixa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryTurnosidSessaoCaixa: TIntegerField
      FieldName = 'idSessaoCaixa'
      Origin = 'idSessaoCaixa'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object qryTurnosidUsuarioAbertura: TIntegerField
      FieldName = 'idUsuarioAbertura'
      Origin = 'idUsuarioAbertura'
      Required = True
    end
    object qryTurnosidUsuarioFechamento: TIntegerField
      FieldName = 'idUsuarioFechamento'
      Origin = 'idUsuarioFechamento'
    end
    object qryTurnosidEstacao: TIntegerField
      FieldName = 'idEstacao'
      Origin = 'idEstacao'
      Required = True
    end
    object qryTurnosdhAbertura: TDateTimeField
      FieldName = 'dhAbertura'
      Origin = 'dhAbertura'
    end
    object qryTurnosdhFechamento: TDateTimeField
      FieldName = 'dhFechamento'
      Origin = 'dhFechamento'
    end
    object qryTurnosvlInicial: TCurrencyField
      FieldName = 'vlInicial'
      Origin = 'vlInicial'
    end
    object qryTurnosvlVenda: TCurrencyField
      FieldName = 'vlVenda'
      Origin = 'vlVenda'
    end
    object qryTurnosTaxaServico: TFloatField
      FieldName = 'TaxaServico'
      Origin = 'TaxaServico'
    end
    object qryTurnosDataEnvioCentral: TDateTimeField
      FieldName = 'DataEnvioCentral'
      Origin = 'DataEnvioCentral'
    end
    object qryTurnosflEnviadoCentral: TBooleanField
      FieldName = 'flEnviadoCentral'
      Origin = 'flEnviadoCentral'
    end
    object qryTurnosUsuarioAbertura: TStringField
      FieldName = 'UsuarioAbertura'
      Origin = 'UsuarioAbertura'
      Required = True
      Size = 80
    end
    object qryTurnosUsuarioFechamento: TStringField
      FieldName = 'UsuarioFechamento'
      Origin = 'UsuarioFechamento'
      Required = True
      Size = 80
    end
  end
  object dsTurnos: TDataSource
    DataSet = qryTurnos
    Left = 608
    Top = 176
  end
end
